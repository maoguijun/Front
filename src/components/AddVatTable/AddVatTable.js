...
//添加选择项，click row 自动选择,
 class SmallSelectionTable extends React.PureComponent {
  constructor(props){
    super(props)
    this.state={
      selectedRowKeys:[],
    ...
    this.handleAdd = props.handleAdd.bind(this)
  }
  onChange=()=>{
  ...
  handleUp() {
    ...
    //为了保证选择的数据是连续的无奈之举
    if(this.state.selectedRowKeys.length ==(this.state.selectedRowKeys[this.state.selectedRowKeys.length-1]-this.state.selectedRowKeys[0]+1)){
  ...
  render() {
    ...
    const props={
      ...
      onRowClick: (record, index,e) => {
        ...
        //有数据的行才执行下面的选择操作
        //不能再其他标签上触发,防止填表单的时候触发这个事件
        if (record.size>1&&e.target.nodeName ==='TD') {
      ...
      < SmallTable {...props }
        ...
        footer={()=>(
          <div>
            ...
            {this.props.upDown&&<ButtonGroup style={{marginLeft:'16px'}}>
 ...
 *
 * **/
export class AddVatTable extends React.PureComponent{
  static defaultProps={
